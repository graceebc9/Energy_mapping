...
def main():
    ...
    if not train_path or not output_path or not test_path:
        print("Please set DATA_PATH and OUTPUT_PATH environment variables.")
        sys.exit(1)

    dataset_name = os.path.basename(train_path).split('.')[0].split('_tr')[0]
    ...
    if train_subset_prop != 1:
        test_subset, _ = train_test_split(test_data, test_size=1-train_subset_prop, random_state=42)
        # train_subset.to_csv(os.path.join(output_directory, 'train_subset.csv'), index=False)
    else:
        test_subset = test_data   
    test_subset = transform(TabularDataset(test_subset), label)
    y_pred = predictor.predict(test_subset.drop(columns=[label]))
    results = predictor.evaluate_predictions(y_true=test_subset[label], y_pred=y_pred, auxiliary_metrics=True)

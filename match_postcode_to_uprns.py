...
def process_ons_file(ons_file, link, raw_elec_pc_lk, gas_elec, fuel_year):
    lab = ons_file.split('_')[-1].split('.')[0]
    print('starting ', lab)
    out_dir = f'./data/fuel_{fuel_year}/{gas_elec}_link'
    os.makedirs(out_dir, exist_ok=True) 
    out_file = os.path.join( out_dir, f'raw_{gas_elec}_uprn_link_{lab}.csv' ) 
    ...
    # check columns to merge are same type 
    ...
    elec_df_uprn = join_pc_map_three_pc_two( uprn_df_link, 'PCDS', 'pcds_2', raw_elec_pc_lk)
    

    elec_df_uprn[['upn', 'Postcode' , 'PCDS', 'pcds_2', 'LSOA21CD', 'LAD22CD_y', 'RGN22CD_x',]].to_csv(out_file)
    print('File saved for ' , lab)
